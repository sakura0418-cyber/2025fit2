...
SCREEN_WIDTH = 256  # 画面の横の長さ
SCREEN_HEIGHT = 256 #画面の縦の長さ

MAX_BUBBLE_SPEED = 1.8　　#bubbleの最高速度
NUM_INITIAL_BUBBLES = 50　#bubble の最初の数
NUM_EXPLODE_BUBBLES = 11　#割れるbubble の数
...
class Bubble:   bubble のクラス
    def __init__(self):　
        self.r = pyxel.rndf(3, 10)
        self.pos = Vec2(
            pyxel.rndf(self.r, SCREEN_WIDTH - self.r),　 ランダム
            pyxel.rndf(self.r, SCREEN_HEIGHT - self.r),　ランダム
        )
        self.vel = Vec2(
            pyxel.rndf(-MAX_BUBBLE_SPEED, MAX_BUBBLE_SPEED),　ランダム
            pyxel.rndf(-MAX_BUBBLE_SPEED, MAX_BUBBLE_SPEED),　ランダム
        )
        self.color = pyxel.rndi(1, 15)　bubbleの色（ランダム）
